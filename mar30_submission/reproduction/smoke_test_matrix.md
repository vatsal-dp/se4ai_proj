...
| 4 | DL-CIBuild (rank 12) | https://github.com/stilab-ets/DL-CIBuild | Full Docker run (`tensorflow/tensorflow:2.16.1`) executing `evaluate_tuner(\"default\", train_fold_0)` + fold-0 test prediction on `dataset/cloudify.csv` | Pass (Docker) | Reproduced baseline metrics and saved artifact `dl_cibuild_rank12_output.json`; run recipe and compatibility notes in `dl_cibuild_rank12_run.md` |
...
- Candidate 4 (DL-CIBuild) is reproducible in Docker after compatibility hardening for optional tuner dependencies; a baseline fold output was generated and archived.
